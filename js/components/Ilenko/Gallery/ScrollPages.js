...
define({
  ...
  renderScrollPages: function (funcName, numberPage, pagesCount, userID) {
    ...
    function numBlock(funcName, number, userID) {
      let numBlockString = `<div class="scrollPages_number"">${number}</div>`;
    ...
    // начинается рассчет блоков, если количество страниц больше 7
      ...
      // в результате бубут выведены следующие данные: 1  ... {текущая страница}-{последняя страница}
      else if (numberPage >= 5 && numberPage >= pagesCount - 3) {
    ...
    let scrollPagesString =
      '<div class="scrollPages"><p class="scrollPages_caption">Выберите страницу: </p>';
